...
class Library:
    ...
    def borrow_book(self, user, book):
        ...
        try:
            ...
            if len(user.borrowed_books) >= Config.BORROW_LIMIT:
                return {"success": False, "message": f"User has reached the borrow limit of {Config.BORROW_LIMIT} books"}
            ...
            if not user.add_book(book):
                # Rollback book status if user update fails
                book.return_book()
                return {"success": False, "message": "Failed to update user's borrowed books"}

            return {"success": True, "message": f"Book '{book.title}' successfully borrowed by {user.username}"}
        except Exception as e:
            print(f"An error occurred while borrowing book: {e}")
    ...
    def return_book(self, user, book):
        ...
        try:
            ...
            # The borrowed_books list might contain strings or dictionaries
            book_found = False
            for borrowed_book in user.borrowed_books:
                if isinstance(borrowed_book, dict) and borrowed_book.get('title') == book.title:
                    book_found = True
                    break
                elif borrowed_book == book.title:
                    book_found = True
                    break

            if not book_found:
                return {"success": False, "message": "User has not borrowed this book"}

            # Update user's borrowed books
            if not user.remove_book(book):
                return {"success": False, "message": "Failed to update user's borrowed books"}

            # Update book status
            if not book.return_book():
                # Rollback user's borrowed books if book update fails
                user.add_book(book)
                return {"success": False, "message": "Failed to update book status"}

            return {"success": True, "message": f"Book '{book.title}' successfully returned by {user.username}"}
        except Exception as e:
            print(f"An error occurred while returning book: {e}")
            return {"success": False, "message": f"An unexpected error occurred: {str(e)}"}
